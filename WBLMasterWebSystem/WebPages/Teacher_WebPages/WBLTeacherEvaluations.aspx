...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <asp:SqlDataSource id="studentEvalCommand" ConnectionString="<%$ ConnectionStrings:connectionTom %>" SelectCommand="Select CONVERT(varchar(10),M.MemberID) + '-' + M.FirstName + ' ' + M.LastName + ',' + M.Email as [Name] From Member M JOIN Student S on M.MemberID = S.StudentID JOIN ClassSchedule CS ON CS.StudentID = S.StudentID JOIN Course C on C.CourseID = CS.CourseID JOIN Staff ST ON C.TeacherID = ST.StaffID Where ST.StaffID = 4 AND C.CourseID = @CourseID" runat="server" >
        <SelectParameters>
            <asp:SessionParameter Name="CourseID" SessionField="EvaluationCourseID" Type="Int32" />
        </SelectParameters>
    </asp:SqlDataSource>
    <asp:SqlDataSource id="courseListCommand" ConnectionString="<%$ ConnectionStrings:connectionTom %>" SelectCommand="SELECT CONVERT(varchar(10),[CourseID]) + '-' + [CourseName] as [CourseTitle] FROM [Course] WHERE ([TeacherID] = @TeacherID)" runat="server" DeleteCommand="DELETE FROM [Course] WHERE [CourseID] = @CourseID" InsertCommand="INSERT INTO [Course] ([CourseName]) VALUES (@CourseName)" UpdateCommand="UPDATE [Course] SET [CourseName] = @CourseName WHERE [CourseID] = @CourseID" >
        <DeleteParameters>
